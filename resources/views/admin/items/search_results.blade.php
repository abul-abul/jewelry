            ...
            <div class="portlet box green">
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <form action="{{action('Admin\ItemController@getSearchItems', [1, $type])}}">
                    ...
                    <div class="tools">
                        ...
                        <a class="delete_selected_items" @if(count($items) > 0) data-count ="{{$items->last()->id}}" @endif style="color: white">/ Delete </a>
                ...
                <div class="portlet-body no-more-tables">
                    ...
                    <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                @endif
                                    <tr class="item">
                                        <td class="item_id" value="{{$item->id}}">{{($page-1)*3 + $key + 1}}</td>
                                        <td>
                        ...
                        @if($page-1 > 0)
                        <form action="{{action('Admin\ItemController@getSearchItems', [$page-1, $type])}}">
                            <input type="hidden" name="search" value="{{$search}}">
                            <a href="#"><button type="submit">Previous</button></a>
                        </form>
                        @endif
                        <button>{{$page}}</button>
                        @if($page < $maxPage)
                        <form action="{{action('Admin\ItemController@getSearchItems', [$page+1, $type])}}">
                            <input type="hidden" name="search" value="{{$search}}">
                            <a href="#"><button type="submit">Next</button></a>
                        </form>
                        <!-- <a href="{{URL::to('admin/item/search-items', [$page+1, $type])}}"><button>Next</button></a> -->
                        @endif
                        </div>
                    @else
                        <h3>No Items</h3>
...
{!! HTML::script(asset('assets/js/check_all.js')) !!}
    <script type="text/javascript">
        $(".show_modal" ).click(function() {
            ...
            $('#delete_item').attr('href', '/admin/item/delete-item/'+id);
