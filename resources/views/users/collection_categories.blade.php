	...
	<div class="container">
		<div class="page-top-in">
			<ol class="breadcrumb pull-left">
				...
				<li>
					<a href="{{URL::to('item/items',['collection', $breadcrumb, 'noSort'])}}">{{$breadcrumb}}</a>
				</li>
				<li>
					<a href="{{URL::to('item/items',['category', $type, 'noSort'])}}">{{$type}}</a>
				</li>
			</ol>
			<div class="breadcrumb pull-right">
				<div class="btn-group" >
	               ...
	               <ul  class="dropdown-menu sort" role="menu" >
	                   <li>
	                       <a href="{{URL::to('item/collection-categories',[$breadcrumb, $type, 'name'])}} " >@if($sort == 'name')<i style="color:white" class="fa fa-check"></i>@endif NAME</a>
	                   </li>
	                   <li>
	                       <a  href="{{URL::to('item/collection-categories',[$breadcrumb, $type, 'price'])}}" >@if($sort == 'price')<i style="color:white" class="fa fa-check"></i>@endif PRICE</a>
	                   </li>
	                   <li>
	                       <a   href="{{URL::to('item/collection-categories',[$breadcrumb, $type, 'noSort'])}}" >@if($sort == 'noSort')<i style="color:white" class="fa fa-check"></i>@endif POSITION</a> 
	                   </li>
	               </ul>
	           </div>
		        <div class="btn-group " >
														...
														<span class="product-thumb-info-act">
															@if(Auth::check())
																@if(isset($item->favorite))
																	<a class="list_heart add-to-favorites" href="#"  data-item = "{{$item->id}}" class="list_heart added-to-favorites" data-status="1">
																	<span><i class="icon fa fa-heart"> ADDED</i></span> 
																	</a>
																@else
																	<a class="list_heart add-to-favorites" href="#"  data-item = "{{$item->id}}" data-status="0">
															...
															@else
															<a class="add-to-cart-product item_list{{$item->id}}" data-id="{{$item->id}}" data-subtota = "{{$subtotal}}" data-title = "{{$item->title}}" data-category = "{{$item->category->category}}" data-price = "{{$item->new_price}}" href="#" data-toggle="modal" data-status="{{$item->status}}">
